...
#define SLOAD_H
	...
	return tex2D (s_base, tc);
		...
		return S;
			...
			float4 Nu=tex2D	(s_bump, new_tc);
			float4 NuE=tex2D (s_bumpX, new_tc);
			S.base=tbase (new_tc);
			S.normal=Nu.wzyx+(NuE.xyz-1.0h);
			//	S.normal = half3(0.00001,0.00001,0.00001);
			if (L_hemi_color.x == 0 && L_hemi_color.w == 1)
				S.normal = half3(0.00001,0.00001,0.00001);
			S.gloss=Nu.x*Nu.x;
			S.height=NuE.z;
			#ifdef USE_TDETAIL
			...
			{
			surface_bumped S;
			float4 Nu=	tex2D (s_bump, I.tcdh);
			float4 NuE=	tex2D (s_bumpX,I.tcdh);
			S.base=	tbase (I.tcdh);
			S.normal=	Nu.wzyx+(NuE.xyz-1.0h);
			S.gloss=	Nu.x*Nu.x;
			S.height=NuE.z;
