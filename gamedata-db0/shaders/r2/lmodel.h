...
float4 plight_infinity2(float m,float3 pnt,float3 normal,float3 light_direction){
float3 N=normal;
float3 V=-normalize(pnt);
float3 L=-light_direction;
float3 H=normalize(L+V);
...
s*=f;
float4 r=tex3D(s_material,float3(dot(L,N),s,m));
...
float4 plight_infinity(float m,float3 pnt,float3 normal,float4 light_dir) {
float3 T=tan(normal);
float3 N=normalize(normal);
float3 V=-normalize(pnt);
float3 L=-light_dir.xyz;
float3 tx=normalize(T);
float3 H=normalize(L+V);
float CP=get_val();
...
return (ret);}
uniform float pre_alpha;
uniform float this_is_not_supposed_to_be_here;
float4 plight_local(float m,float3 pnt,float3 normal,float4 light_position,float light_range_rsq,out float rsqr){
float3 N=normalize(normal);
float3 L2P=pnt-light_position.xyz;
float3 V=-normalize(pnt.xyz);
float3 L=-normalize((float3)L2P);
const float RI=0.01f;
...
float att=saturate(1-rsqr*light_range_rsq);
return (att)*(ret);}
float4 plight_infinity2(float m,float3 pnt,float3 normal,float4 light_dir,float3 X,float4 C0){
float3 N=normalize(normal);
float3 V=-normalize(pnt);
float3 L=-light_dir.xyz;
float3 tx=X;
...
float4 Diff=tex3D(s_material,(float3(LV.w,E2,m)));
if (m<=1.0f) 
return tex3D(s_material,(float3(LV.w,LV.x,m)));
else
if (m==4.0f)
...
float4 plight_local2(float m,float3 pnt,float3 normal,float4 light_position,float light_range_rsq,out float rsqr,float3 X,float4 C0){
float3 N=normalize(normal);
float3 L2P=pnt-light_position.xyz;
float3 V=-normalize(pnt.xyz);
float3 L=-normalize((float3)L2P);
...
return (att)*(float4(tex3D(s_material,(float3(LV.w,LV.x,m)))));
}
#endif
