			...
			{					
				...
				if (true)
				{
					...
					if (WET_DEBUGVAL > 0.3 || (vert > min_vert) )  // was: min_vert
					{
						...
						{
							float contrib=1.h;
							float total_blur = amount * WET_REFBLUR;
							float f=0.f;
							float inc=total_blur/WET_REFBLURQUALITY;
							float3 sum = 0;
							//for (int i=0;i<WET_REFBLURQUALITY;i++){sum+=gaussbloom(float2(newX, newY),f).xyz;contrib++;f+=inc;}
							for (int i=0;i<WET_REFBLURQUALITY;i++){sum+=gaussblur(float2(newX, newY),f).xyz;contrib++;f+=inc;}
					...
					#ifdef WET_BLUR
					float contrib=1.h;
					float total_blur = amount * WET_BLUR; // e_barrier.y * 1000; //1000 * (e_kernel.x-0.4); // --> 0.401
					float f=0.f;
					float inc=total_blur/WET_BLURQUALITY;
					float3 sum = 0;
					for (int i=0;i<WET_BLURQUALITY;i++){sum+=gaussblur(uv,f).xyz;contrib++;f+=inc;}
					Color.rgb += (sum/contrib) - tex2Dlod(s_image,float4(uv,0,0)).xyz;  
					#endif
